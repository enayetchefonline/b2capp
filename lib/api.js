...
export const userRegisterApi = async (formData) => {
	try {
		const params = {
			...
			want_newslatter: formData.want_newslatter || 'yes',
			want_text_message: formData.want_text_message || 'yes',
		};
...
// 			Object.entries(params)
// 				.map(([key, value]) => {
// 					// ✅ Don't encode OrderList — use raw value
// 					if (key === 'OrderList') {
// 						return `${key}=${value}`;
// 					}
// 					if (value === null || value === undefined || value === '') {
...
export const deleteProfileRequest = async ({user_id}) => {
	try {
		const params = {
			funId: 147,
			user_id,
